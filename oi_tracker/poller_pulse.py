...
async def poll_once(nfo_df) -> dict:
    ...
    if r.status_code != 200:
        return _err(f"Kite API {r.status_code}: {r.text[:200]}")

    spot_q = r.json().get("data", {}).get(NIFTY_SPOT_TOKEN, {})
    spot = spot_q.get("last_price")
    if spot is None:
        return _err("Spot price missing in Kite response.")
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            r2 = await client.get(
                f"{KITE_BASE}/quote",
                params=[("i", tok) for tok in token_map],
                headers=headers,
            )
    except Exception as exc:
    ...
    if r2.status_code != 200:
        return _err(f"Kite API {r2.status_code}: {r2.text[:200]}")

    opt_data = r2.json().get("data", {})
